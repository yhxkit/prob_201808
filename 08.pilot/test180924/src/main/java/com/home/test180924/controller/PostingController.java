...
public class PostingController {
    ...
    public Map getAllPosts(HttpServletRequest request){
        ...
        //여기 그냥 page로 가져와서 프론트에서 total 프로퍼티로.......
    ...
    public ResponseEntity write(HttpServletRequest request, @RequestBody PostDto post){ // postman으로 할때는 @RequestBody 빼야하고...
        System.out.println("포스트 작성 "+ post.getTitle()+" "+post.getPostContent());
    ...
    public ResponseEntity<?> reply(@RequestBody CommentDto commentDto ,@PathVariable("1") int postIdx, HttpServletRequest request ){
        System.out.println(postIdx+"번 포스트의 코멘트 작성 "+commentDto.getCommentContent() + commentDto.getCommentWriterEmail());
    ...
    public ResponseEntity<?> editComment(Comment comment, @PathVariable("1") int postIdx, @PathVariable("2") int commentIdx){
        System.out.println(postIdx+"번 포스트의 "+comment.getCommentIdx()+" 코멘트 수정 "+ comment.getCommentContent());
