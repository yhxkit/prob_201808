...
대부분 두가지의 접근 방식이 유사하게 동작하지만, IE 모델은캡처링이 안되고 버블링만 된다.
###### 자식>부모 : 버블링, 부모>자식 : 캡처링


IE모델은 설적된 이벤트 핸들러에 콘텍스트를 제대로 설정하지 않는다. 이로 인해 핸들러 내부에서 대상 엘리먼트 대신에 전역 콘텍스트를 참고하게 된다. 게다가 IE모델은 이벤트 정보를 핸들러에 전달하지 않고 전역 콘텐스트인 window 객체에 덧붙여 놓았다.
...
따라서 데이터블록이handlers 라는 프로퍼티를 가지고 있는지 확안하고 없으면 추가한다. 
...
이벤트 핸들러들을 호출할 대마다 이벤트가 발생한 엘리먼트를 함수의 콘텍스트로 제공하고, 이벤트 객체를 유일한 인자로 제공한다.
...
이벤트 핸들러를 설정하는 메서드를 마들었으니, 해제하는 것도 필요하다. 핸들러들을 직접 설정하지 않고 대표 핸들러를통해서 이벤트 처리 과정에 대한 제어가 가능한 방식을 택했으니 브라우저가 제공한느 이벤트 해제 함수들을 이용할 수 없 다.   
