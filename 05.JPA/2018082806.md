...
메서드 명이 다소 길거나 복잡하다면 @Query 애노테이션을 사용할수 있다..


## 05 Specification을 이용한 검색 조건 조합 


14장의 크리테리아 API를 이용해서 검색조건인 Predicate 를 조합할수 있다. 이에 CriteriaBuilder 가 필요하며, Entitymanager로 생성할 수 있다.

Specification 인터페이스로 검색 조건을 조합하고, 조건으로 검색하는 리포지토리를 작성하는 방법은 

* 리포지토리 인터페이스에 Specification을 파라미터로 받는 메서드 정의
* 검색 조건을 생성하는 Specification 구현하기
* 검색 조건을 조합한 Specification 객체로 검색하기
...
스프링 데이터 JPA 사용하기~  
extends Repository<엔티티, String>한 인터페이스로 메서드 추가 
